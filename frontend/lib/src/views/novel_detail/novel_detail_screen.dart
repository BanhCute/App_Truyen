...
class _NovelDetailScreenState extends State<NovelDetailScreen> {
  ...
  Future<void> toggleFollow() async {
    try {
      ...
      final currentFollowCount = novel.followerCount;
      ...
      setState(() {
        ...
        novel = novel.copyWith(
          followerCount:
              wasFollowing ? currentFollowCount - 1 : currentFollowCount + 1,
        );
      });
      ...
      try {
        ...
        final updatedNovel =
            await NovelService.updateNovelInfo(widget.novel.id);

        if (mounted) {
          setState(() {
            novel = updatedNovel;
            // Không cập nhật lại isFollowing vì đã được set đúng ở trên
          });
        ...
        if (mounted) {
          setState(() {
            isFollowing = wasFollowing;
            novel = novel.copyWith(
              followerCount: currentFollowCount,
            );
          });
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  'Có lỗi xảy ra: ${e.toString().replaceAll('Exception: ', '')}'),
              duration: const Duration(seconds: 2),
            ),
          );
        }
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  Text(
                                    getTimeAgo(
                                        novel.updatedAt.toIso8601String()),
                                    style: const TextStyle(
                                      color: Colors.white70,
                                      fontSize: 14,
                                    ),
